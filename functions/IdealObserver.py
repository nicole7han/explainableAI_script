...
targets = np.random.multivariate_normal(t_mean, t_cov, N).round()
for i in range(N):
    x1, x2, x3 = targets[i, :]
    stinuli, stimuli_n = get_stimuli(x1, x2, x3, imageSizeX, imageSizeY)
    # probability target/distractor given the image


x = [80,40,41]
p = get_pdf(x, t_mean, t_cov)
cdf = get_cdf(x,t_mean,t_cov)
...
def IO(imageSizeX, imageSizeY, means, covs, feature_range, n_trials, sigma=.08, contrast=1):
        ...
        for x1 in np.arange(feature_range[c][0][0], feature_range[c][0][1]+1):
            for x2 in np.arange(feature_range[c][1][0], feature_range[c][1][1] + 1):
                for x3 in np.arange(feature_range[c][2][0], feature_range[c][2][1] + 1):
                    ...
                    p_sk = get_pdf([x1,x2,x3],means[c],covs[c])*prior
    ...
    for c in range(n_class):
        mean = means[c]
        cov = covs[c]
        samples = np.random.multivariate_normal(mean, cov, n_trials).round()
    ...
    for c in range(n_class):
        ...
        p_gs = [] #likelihood for each sample image for class c
        for i in range(n_samples): # for each individual sample image
            sample = samples[i]
            # vector of p(g|si,k)
            p_gk = np.exp( (-np.repeat(np.array(dot_sum(sample,sample)), n_signals, axis=0) \
                  + 2*np.dot(signals, sample) \
                  - (signals*signals).sum(1)) / (2*sigma**2))
            p_gs.append((p_gk * signals_p).sum())
